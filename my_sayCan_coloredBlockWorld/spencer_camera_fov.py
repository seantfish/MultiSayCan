...
class CartpoleRobot(RobotSupervisorEnv):
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array([-0.4, -np.inf]),
                                     high=np.array([0.4, np.inf]),
                                     dtype=np.float64)
        # Define agent's action space using Gym's Discrete
        self.action_space = Discrete(2)

        self.robot = self.getSelf()  # Grab the robot reference from the supervisor to access various robot methods
        #self.camera = self.getDevice("good_camera")
        imu = self.getDevice("inertial_unit")
        #self.camera.enable(self.timestep)
        #self.blue_camera = self.getDevice("blue_camera")
        #self.blue_camera.enable(self.timestep);
        #self.blue_camera.recognitionEnable(self.timestep)
        
        self.red_camera = self.getDevice("red_camera")
        self.red_camera.enable(self.timestep);
        self.red_camera.recognitionEnable(self.timestep)
        ...
        #self.camPos = self.getDevice("camera")
    ...
    def get_camera_field_of_view(self):
        ...
        #print("get camera")
        blue_object = self.red_camera.getRecognitionObjects()
        
        
        if  len(blue_object) >=1:
            blue_object = blue_object[0]
            color_ar = blue_object.getColors()
            #print("blue Object")
            red_chan = color_ar[0]
